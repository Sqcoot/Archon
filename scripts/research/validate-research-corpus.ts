...
async function validateResearchCorpus(): Promise<ValidationResult> {
  ...
  const graphEvidenceIndex = await readGraphEvidenceIndexRows();
  ...
  for (const definition of REQUIRED_UPSTREAMS) {
    ...
    const trackedIndexProof = graphEvidenceIndex.get(definition.name);
    if (entry.graphStatus === 'complete' && !graphExists && trackedIndexProof !== 'complete') {
      errors.push(
        `${definition.name} graphStatus=complete but graph outputs are missing and tracked graph evidence index does not prove completion.`
      );
    }
...
async function readGraphEvidenceIndexRows(): Promise<Map<string, string>> {
  ...
  for (const line of raw.split('\n')) {
    ...
    const cells = line
      ...
      .slice(1, -1)
      .map(cell => cell.trim());
    if (cells.length < 8 || cells[0] === 'Repository') continue;
