...
function buildToolAvailabilityEntries(
  ...
  return [
    ...
    toolEntry({
      id: 'tool.aco-capability-snapshot',
      name: 'ACO capability snapshot',
      category: 'aco',
      status: 'available',
      sourceEvidence: 'packages/context-orchestrator/src/capability-snapshot.ts',
      invocationPath: 'buildCapabilitySnapshot({ cwd, prompt })',
      scope:
        'Evidence-backed capability discovery for providers, commands, workflows, MCP, plugins, hooks, roles, ledgers, artifacts, graph, and docs targets.',
      preconditions:
        'Repository files are readable; active auth and user-level provider config remain out of scope.',
      verification: 'Capability snapshot adapter module is present and fixture-testable.',
      primaryUse:
        'Load compact ACO capability evidence into prompt/session/subagent/tool-loop context.',
      failureMode:
        'Missing manifest, unsupported provider evidence, or secret-like value in source evidence.',
      fallback: 'Mark claim unknown, blocked, or deferred and route through existing ACO ledgers.',
      owner: 'context-orchestrator',
      lastVerified,
      lastVerifiedAt,
      notes: 'Read-only adapter; no active hook/config mutation.',
      confidence: 'declared',
    }),
    toolEntry({
...
function buildCommandEntries(
  ...
  const commands: CommandLedgerEntry[] = [
    ...
    commandEntry({
      id: 'cmd.aco-bootstrap-context',
      command: 'buildAcoBootstrapContext({ cwd, prompt, event, maxBytes })',
      category: 'aco',
      status: 'available',
      sourceEvidence: 'packages/context-orchestrator/src/capability-snapshot.ts',
      invocationPath: 'context-orchestrator package API',
      scope: 'Generate compact event-specific ACO bootstrap context.',
      preconditions: 'Capability snapshot can be built from read-only repository evidence.',
      verification: 'Bootstrap context API is exported and covered by acceptance tests.',
      primaryUse: 'Inject or hand off compact ACO context at lifecycle boundaries.',
      failureMode:
        'Unsupported lifecycle event, oversized context, or unknown capability evidence.',
      fallback: 'Use capability-snapshot.json and ledgers directly.',
      owner: 'context-orchestrator',
      lastVerified,
      lastVerifiedAt,
      mutatesTrackedFiles: false,
