...
describe('aco-context import boundary', () => {
  test('production source is pure and imports only committed ACO contracts, zod, and local modules', async () => {
...
async function collectSourceFiles(directoryUrl: URL): Promise<string[]> {
  ...
  const files = await Promise.all(
    entries.map(async entry => {
      ...
      if (entry.isDirectory()) return collectSourceFiles(new URL(`${entry.name}/`, directoryUrl));
