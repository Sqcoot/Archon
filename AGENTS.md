...
- Use `archon context route` before broad or ambiguous implementation tasks.
- Use `archon context compile` before architecture-sensitive implementation.
- Use OpenAI Docs MCP for OpenAI/Codex behavior.
- Use Context7 MCP for third-party library/API documentation.
- Use Graphify when repository structure matters.
- Follow the returned BMAD route.
- Follow SDD: update specs before code.
- Follow ATDD: acceptance scenarios before implementation.
- Use unit tests only after acceptance criteria exist.
- Never expose secrets.
- Keep route, status, and review summaries compact when Caveman policy applies.
